#!/usr/bin/env bash
# operation_cucbrspmm.sh <outdir> [key=value ...]
# NVIDIA cuSPARSE BSR SpMM wrapper
set -euo pipefail
...
# Use the Python cuSPARSE BSR SpMM implementation
CUSPARSE_SCRIPT="$PROJECT_ROOT/scripts/cucbrspmm.py"
if [[ ! -f "$CUSPARSE_SCRIPT" ]]; then
    echo "Error: cuSPARSE BSR SpMM script not found at $CUSPARSE_SCRIPT" >&2
    echo "TIMING_MS:0"
    exit 1
fi

# Build command line arguments
CUSPARSE_ARGS=("$REORDERED" "--alpha" "$alpha" "--beta" "$beta" "--block-size" "$block_size" "--n-cols" "$n_cols")
...
if echo "$CUSPARSE_OUTPUT" | grep -q "^TIMING_MS:"; then
    ...
    echo "Error: No timing output from cuSPARSE BSR SpMM implementation" >&2
