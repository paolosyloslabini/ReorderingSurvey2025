#!/usr/bin/env python3
"""
Test the enhanced cuSPARSE implementation including GPU environment detection.
"""
import os
import subprocess
import tempfile
...
def test_cusparse_script_gpu_detection():
    """Test that the cuSPARSE script properly detects GPU environment."""
...
"""
    ...
    try:
        script_path = Path(__file__).parent.parent / "scripts" / "cusparse_spmm.py"
        ...
        assert "Using CPU sparse matrix multiplication" in result_cpu.stderr
        
    finally:
        os.unlink(matrix_path)


def test_cucsrspmm_wrapper_with_parameters():
    """Test the updated cucsrspmm wrapper with various parameters."""
...
def test_cucsrspmm_error_handling():
    """Test error handling in cucsrspmm wrapper."""
    with tempfile.TemporaryDirectory() as tmp_dir:
        outdir = Path(tmp_dir) / "output"
        outdir.mkdir()
...
if __name__ == "__main__":
    test_cusparse_script_gpu_detection()
    test_cucsrspmm_wrapper_with_parameters()
    test_cucsrspmm_error_handling()
    print("All enhanced cuSPARSE tests passed!")